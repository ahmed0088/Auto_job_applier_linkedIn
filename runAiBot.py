...
maxApplicationsPerRunReached = False
    ...
    * Returns: `True` if user is logged-in or `False` if not
    ...
    try:
        WebDriverWait(driver, 15).until(lambda d: d.current_url == "https://www.linkedin.com/feed/"
                   ...
                   or try_linkText(d, "Join now"))
    except WebDriverException:
        pass
    if driver.current_url == "https://www.linkedin.com/feed/": return True
    if try_xp(driver, '//input[@id="username"]', False): return False
    ...
    try:
        ...
        pass
    print_lg(f"Didn't find Sign in link, so assuming user is logged in! Current URL: {driver.current_url}, Page title: {driver.title!r}")
    return True
...
def login_LN() -> None:
    ...
    * If both failed, asks user to login manually
    ...
    if username == "username@example.com" and password == "example_password":
        ...
        return
    try:
        # Wait for the username field itself rather than English link text like
        # "Forgot password?" - LinkedIn renders the login page in whatever
        # language the browser profile defaults to, and the field IDs are the
        # only part of the page guaranteed to stay the same across languages.
        ...
        try:
            ...
            # print_lg(e)
        # Find the login submit button and click it (no text match - the
        # button's label is localized, but it's the only type="submit" button
        # on the login form).
...
def get_applications_today_count() -> int:
    '''
    Returns how many applications (from the history CSV) have a "Date Applied"
    matching today's date. Used to enforce `max_applications_per_day` across
    multiple runs/restarts in the same day, not just within a single run.
    '''
...
def apply_filters() -> None:
    ...
    try:
        ...
        # A missing/broken GUI backend (e.g. no Tkinter, common on Homebrew Python on
        # macOS) used to make this dialog raise here uncaught, silently killing the
        # whole run for the rest of this cycle - safe_confirm logs and returns None
        # instead, so a failed filter application just moves on to the next search term.
        safe_confirm(f"Faced error while applying filters. Please make sure correct filters are selected, click on show results and click on any button of this dialog, I know it sucks. Can't turn off Pause after search when error occurs! ERROR: {e}", "Filters Error", ["Doesn't look good, but Continue XD", "Look's good, Continue"])
...
def apply_to_jobs(search_terms: list[str]) -> None:
    ...
    global current_city, failed_count, skip_count, easy_applied_count, external_jobs_count, tabs_count, pause_before_submit, pause_at_failed_question, useNewResume, maxApplicationsPerRunReached
    current_city = current_city.strip()

    if restrict_active_hours and not (active_hours_start <= datetime.now().hour < active_hours_end):
        maxApplicationsPerRunReached = True
        print_lg(f"\n###############  Outside active hours ({active_hours_start}:00-{active_hours_end}:00). Not starting this run!  ###############\n")
        return

    if max_applications_per_day and get_applications_today_count() >= max_applications_per_day:
        maxApplicationsPerRunReached = True
        print_lg(f"\n###############  Already reached the configured limit of {max_applications_per_day} applications for today. Not starting this run!  ###############\n")
        return
        ...
        try:
            while current_count < switch_number:
                ...
                for job in job_listings:
                        ...
                        try:
                            try:
                                ...
                                if errored != "stuck" and cur_pause_before_submit:
                                    decision = safe_confirm('1. Please verify your information.\n2. If you edited something, please return to this final screen.\n3. DO NOT CLICK "Submit Application".\n\n\n\n\nYou can turn off "Pause before submit" setting in config.py\n"Disable Pause" skips the pause for THIS application only - it comes back for the next one.', "Confirm your information",["Disable Pause", "Discard Application", "Submit Application"])
                                    if decision == "Discard Application": raise Exception("Job application discarded by user!")
                                    # Note: intentionally NOT persisted to the global pause_before_submit - "Disable
                                    # Pause" only affects cur_pause_before_submit (this application), so pausing
                                ...
                                elif errored != "stuck" and cur_pause_before_submit and "Yes" in (safe_confirm("You submitted the application, didn't you 😒?", "Failed to find Submit Application!", ["Yes", "No"], default="") or ""):
                                    date_applied = datetime.now()
                    ...
                    total_applied_this_run = easy_applied_count + external_jobs_count

                    if max_applications_per_run and total_applied_this_run >= max_applications_per_run:
                        maxApplicationsPerRunReached = True
                        print_lg(f"\n###############  Reached the configured limit of {max_applications_per_run} applications for this run. Stopping!  ###############\n")
                        return

                    if max_applications_per_day and get_applications_today_count() >= max_applications_per_day:
                        maxApplicationsPerRunReached = True
                        print_lg(f"\n###############  Reached the configured limit of {max_applications_per_day} applications for today. Stopping!  ###############\n")
                        return

                    if restrict_active_hours and not (active_hours_start <= datetime.now().hour < active_hours_end):
                        maxApplicationsPerRunReached = True
                        print_lg(f"\n###############  Reached the end of active hours ({active_hours_start}:00-{active_hours_end}:00). Stopping for now!  ###############\n")
                        return

                    # Pause between applications to keep a human-like pace instead of rapid, evenly-spaced
                    # submissions, which is the kind of pattern that gets an account flagged for automated
                    # activity. Every so often, take a longer "stepped away for a bit" break instead of the
                    # usual short gap, mimicking how a real person applies in bursts.
                    if extended_break_every and total_applied_this_run % extended_break_every == 0:
                        gap = randint(extended_break_min, extended_break_max) if extended_break_max > extended_break_min else extended_break_min
                        print_lg(f"Taking an extended break of {gap}s after {total_applied_this_run} applications...")
                        sleep(gap)
                    else:
                        gap = randint(min_application_gap, max_application_gap) if max_application_gap > min_application_gap else min_application_gap
                        if gap > 0:
                            print_lg(f"Pausing {gap}s before the next application to keep a human-like pace...")
                            sleep(gap)
...
def run(total_runs: int) -> int:
    if dailyEasyApplyLimitReached or maxApplicationsPerRunReached:
        return total_runs
    print_lg("\n########################################################################################################################\n")
    ...
    print_lg("########################################################################################################################\n")
    if not dailyEasyApplyLimitReached and not maxApplicationsPerRunReached:
