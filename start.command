...
# Homebrew-installed Python doesn't ship its own trusted certificate bundle
# (unlike python.org installers), which makes HTTPS requests fail with
# CERTIFICATE_VERIFY_FAILED. Point it at certifi's bundle so downloads
# (e.g. the Chrome driver) work.
export SSL_CERT_FILE="$("$VENV_PY" -m certifi 2>/dev/null)"
