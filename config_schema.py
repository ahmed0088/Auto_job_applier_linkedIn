    ...
    },
    {
        ...
        "fields": [
            ...
            _f("Run settings", "settings", "max_applications_per_day", "Max applications per day", "number",
               "Also cap total applications across the whole day (counts today's entries in your applied-jobs history, even across restarts), not just this one run. Set to 0 for no limit."),
            _f("Run settings", "settings", "min_application_gap", "Min pause between applications (seconds)", "number",
               "After each application, wait at least this many seconds before starting the next one. Helps keep activity looking human-paced instead of rapid-fire."),
            _f("Run settings", "settings", "max_application_gap", "Max pause between applications (seconds)", "number",
               "After each application, wait at most this many seconds (a random amount between the min and max above) before starting the next one."),
            # --- advanced run options ---
            _f("Run settings", "settings", "restrict_active_hours", "Only apply during set hours", "bool",
               "Only submit applications between the start and end hour below. Outside that window the bot stops for the day instead of applying at odd hours, which is an easy tell that a script is running.", advanced=True),
            _f("Run settings", "settings", "active_hours_start", "Active hours start (0-23)", "number",
               "Hour of day (24-hour, your computer's local time) applications are allowed to start. Only used when 'Only apply during set hours' is on.", advanced=True),
            _f("Run settings", "settings", "active_hours_end", "Active hours end (0-23)", "number",
               "Hour of day applications must stop by. Only used when 'Only apply during set hours' is on.", advanced=True),
            _f("Run settings", "settings", "extended_break_every", "Extended break every N applications", "number",
               "Take a longer break after this many applications instead of the usual short pause - mimics applying in bursts with breaks in between, like a real person would. Set to 0 to disable.", advanced=True),
            _f("Run settings", "settings", "extended_break_min", "Extended break minimum (seconds)", "number",
               "Minimum length of the extended break above, in seconds.", advanced=True),
            _f("Run settings", "settings", "extended_break_max", "Extended break maximum (seconds)", "number",
               "Maximum length of the extended break above, in seconds.", advanced=True),
